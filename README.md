...
个股页的“参考价格区间历史”优先显示从 `2024-09-24` 起的收盘价折线叠加估值区间图，并用虚线标出当前价格。
...
运行节奏：一旦启动，持续运行到队列清空。每只股票研究完成、入库并验证后，等待 10 分钟再领取下一只。
...
- `/research/bulk`：批量主动研究入口；从粘贴文本中提取 `000858.SZ`、`SZ000858`、`SH688041`、`688041.SH` 或可推断交易所的 6 位代码并加入待研究队列。
...
首页末尾会展示“接口说明”模块，摘要显示公开接口数量、推荐入口、功能分组和安全边界。除 `/research?stock={code}` 与 `/research/bulk` 可能写入本地研究队列外，数据接口均为只读；系统不输出交易指令、现金金额或股数。

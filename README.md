...
每天从 `https://leader.okbbc.com/api/index` 读取 `key_results.primary_output.items` 中的 `A 可跟踪龙头`，同时从 `https://theme.okbbc.com/api/index` 读取主线环境快照；Leader 只决定哪些股票入队，Theme 提供市场状态、主线强度、生命周期、ETF/板块热度、拥挤和风险偏好背景。战略深研一般只做一次，财务估值深研可以多次刷新，并在个股页叠加历史估值区间。

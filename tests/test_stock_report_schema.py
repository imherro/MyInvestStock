...
def valid_stock_research_sample() -> dict[str, object]:
    return {
        ...
        "multi_bagger_potential": "数倍潜力取决于利润复合增长和估值中枢修复。",
...
class StockReportSchemaTests(unittest.TestCase):
    def test_valid_sample_passes_schema(self) -> None:
        report = validate_stock_research_report(valid_stock_research_sample())
    ...
    def test_invalid_sample_rejects_extra_dict_passthrough(self) -> None:
        sample = valid_stock_research_sample()
    ...
    def test_db_insert_rejects_raw_dict_and_accepts_validated_report(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with closing(connect(db_path)) as conn:
                with self.assertRaises(TypeError):
                    insert_research_run(conn, valid_stock_research_sample())  # type: ignore[arg-type]
        ...
        self.assertEqual(row["task_type"], "stock_research")

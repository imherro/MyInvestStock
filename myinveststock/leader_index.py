...
- 可包含 title, summary, industry_position, competition_landscape, upstream_downstream, annual_growth, multi_bagger_potential, evidence, assumptions 作为解释性输入，但这些字段不能覆盖 deterministic valuation 或 conclusion，也不能覆盖 MyInvestLeader/MyInvestTheme 上游信号。
...
- 先读取 Leader /api/index，只使用 key_results.primary_output.items 中匹配 {code} 的记录作为个股入口。

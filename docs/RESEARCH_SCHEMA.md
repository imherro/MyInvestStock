...
新入库研究结果必须先通过 `core/schema/stock_report.py` 中的 `StockResearchReport` 校验。旧两阶段数据不再兼容，数据库初始化会删除旧队列、旧任务状态和旧估值记录。
...
- `stock_research` 必须写入完整估值区间，且 `low <= mid <= high`。
...
- `stock_code`、`task_type`：任务对象，其中 `task_type` 固定为 `stock_research`。
...
- `trigger_reason` 记录本次为什么入队，例如 `新进入可跟踪龙头`、`手工请求研究`、`估值中枢变化`。
...
个股深研中的估值数值不应由 LLM 临场生成。数值层由 `core/valuation` 输出：
...
只有 `stock_research` 一种研究记录。触发原因决定为什么重研，但不拆分不同流程。触发频率通常来自新进入可跟踪龙头、手工请求、财报更新、重大事件、估值中枢变化或定期复核。

## 入库方式

```powershell
python scripts/import_research_run.py research/stocks/600519.SH/stock_research_2026-06-23.json
```

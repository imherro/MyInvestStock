...
- `/research?stock={code}` 和 `/research/bulk` 在目录中会明确标为非只读，因为它们可能写入本地研究队列；这些入口仍不产生交易、现金金额或股数。
...
用途：从首页批量粘贴文本中提取股票代码，并加入待研究队列。

行为：

- 使用 `POST` 表单字段 `stocks`。
- 支持 `000858.SZ`、`SZ000858`、`SH688041`、`688041.SH`，以及可推断交易所的 6 位代码。
- 代码会统一归一为 `000858.SZ` 这类标准格式，并去重。
- 本地已有研究记录或队列任务时，不重复入队。
- 只写入本地研究队列，不直接执行深研，不输出交易指令、现金金额或股数。
...
除主动研究入口可能写入本地研究队列外，数据接口都只读；所有接口都不包含交易指令、现金金额或股数。
